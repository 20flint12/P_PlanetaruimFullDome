...
texture Fields2 <string uiname="Fields Texture2";>;
sampler SampFields2 = sampler_state    //sampler for doing the texture-lookup
{
    Texture   = (Fields2);          //apply a texture to the sampler
    //MipFilter = point;         //sampler states
    MinFilter = linear;
    MagFilter = linear;
    addressU = border;
...
float4x4 FieldsTransform2 <string uiname="inverse (Fields transform2 * scale 0.5)";>;
 ...
 */   
    ...
    float2 fieldsAdd2 = tex2D(SampFields2, mul(float4(posQueue.rg,0,1), FieldsTransform2)/2 + 0.5).rg;
	fieldsAdd2.rg += -.5;
	fieldsAdd2 = mul(fieldsAdd2, FieldsRotate) * FieldsValue;

	fieldsAdd = fieldsAdd + fieldsAdd2;
